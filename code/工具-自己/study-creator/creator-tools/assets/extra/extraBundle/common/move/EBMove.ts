 ...
 * 自定义位移动画
 */
export default class EBMove {
    private static movePool: EBPool = new EBPool();
     ...
     * 获取一个运动实例
     */
    public static getInstance(): EBMove {
        let move: EBMove = null;
        move = this.movePool.size > 0 ? this.movePool.get() : new EBMove();
        move.init();
        return move;
    }
     ...
     * 回收一个运动实例
     */
    private static putInstance(item: EBMove) {
        item.onNotUse();
     ...
     * @returns 
     */
    public setParams(target: cc.Node, startNode: cc.Node, endNode: cc.Node, duration: number, isAbsoluteCtrl?: boolean, c1?: cc.Vec2, c2?: cc.Vec2): EBMove {
        this.target = target;
    ...
    public run(call?: Function): EBMove {
