import { _decorator, CCClass, CCFloat, CCString, Color, Graphics, Size, UIRenderer, UITransform } from 'cc';
...
export class CQRCode extends Graphics {

    @property(CCString)
    private _string: string = "Hello World!";
    ...
    @property({})
    private _backColor: Color = new Color();
    ...
    @property({})
    private _foreColor: Color = new Color();
    ...
    public set foreColor(value: Color) {
        ...
        let renderer = this.node.getComponent(UIRenderer) || this.node.addComponent(UIRenderer);
        if (renderer) {
            renderer.color = this.foreColor;
        }
        this.setContent();
    }
    ...
    public set size(value: number) {
        ...
        this._size = value;
        this.setContentSize(value, value);
        this.setContent();
    }

    setContentSize(w: number, h: number) {
        let uiT = this.node.getComponent(UITransform) || this.node.addComponent(UITransform);
        uiT.setContentSize(new Size(w, h));
        this.setContent();
    }
    ...
    setContent() {
        ...
        ctx.clear();
        let width = this.uiTransform.width;
        let offsetX = -width * this.uiTransform.anchorX;
        let offsetY = -width * this.uiTransform.anchorY;
        ctx.rect(offsetX, offsetY, width, width);
    ...
    drawQRCode() {
        ...
        for (let row = 0; row < num; row++) {
            for (let col = 0; col < num; col++) {
                if (qrcode.isDark(row, col)) {
                    this.graphics.rect(offsetX + this.margin + col * tileW, offsetX + size - tileH - Math.round(row * tileH) + this.margin, w, h);
                    this.graphics.fill();
                }
            }
        }
...
CCClass.Attr.setClassAttr(CQRCode, 'lineWidth', 'visible', false);
CCClass.Attr.setClassAttr(CQRCode, 'lineJoin', 'visible', false);
CCClass.Attr.setClassAttr(CQRCode, 'lineCap', 'visible', false);
CCClass.Attr.setClassAttr(CQRCode, 'strokeColor', 'visible', false);
CCClass.Attr.setClassAttr(CQRCode, 'miterLimit', 'visible', false);
CCClass.Attr.setClassAttr(CQRCode, 'fillColor', 'visible', false);